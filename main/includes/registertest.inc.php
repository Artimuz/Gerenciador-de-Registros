<?php
  ...
  if (!isset($_POST['register'])) {
    ...
    /* if(!isset($_SESSION["ponto"])){
      ...
      if (empty($prob)) {
        $falso = "NENHUM PROBLEMA ENCONTRADO";
      }

        if (empty($_POST['serialn']) || empty($_POST['cod']) || empty($_POST['desc']) || empty($_POST['notaf']) || empty($_POST['datanota']) || empty($estado)) {
          ...
          require 'conn.php';
          $sql = "SELECT SN FROM registro WHERE ponto=?";
          ...
          if(!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            exit();
              ...
              if ($resultCheck > 0) {
                ...
                $sql = "INSERT INTO registro (estoque, filial, ponto, dataregistro, cod, SN, notaf, loja, valorloja, valorat, datanota, modelo, aparencia, problema, bateria, adulteracao, coments) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,  ?, ?)";
                $stmt = mysqli_stmt_init($conn);
                mysqli_stmt_bind_param($stmt, "sssssssssssssssssss", $estoque, $filial, $ponto, $datapost, $cod, $serialn, $notaf, $loja, $valorloja, $valorat, $datanota, $desc, $estado, $prob, $capacidadebateria, $falso, $obs);
                mysqli_stmt_execute($stmt);
