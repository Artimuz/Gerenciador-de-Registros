...
<main class="page">
  <div>
    <?php
      ...
      if (isset($_GET['desc'])) {$desc = $_GET['desc'];}else{$desc = "";}
      if (isset($_GET['nota'])) {$nota = $_GET['nota'];}else{$nota = "";}
      if (isset($_GET['data'])) {$data = $_GET['data'];}else{$data = "";}
      if (isset($_GET['loja'])) {$loja = $_GET['loja'];}else{$loja = "";}
      if (isset($_GET['vloja'])) {$vloja = $_GET['vloja'];}else{$vloja = "";}
      if (isset($_GET['vat'])) {$vat = $_GET['vat'];}else{$vat = "";}
      if (isset($_GET['bat'])) {$bat = $_GET['bat'];}else{$bat = "";}
      if (isset($_GET['empty'])) {$erro = "<erro>****</erro>";} else {$erro = "";}


      {
      echo "<form name='register' action='includes/register.inc.php' method='post'>
                <content class='edit'>
                  <div>
                    <box>
                        <b>Número de série:</b>".$erro."<br>
                        <input type='text' name='serialn' value='".$sn."'><br>

                        <b>Código do item:</b></td>".$erro."<br>
                        <input type='text' name='cod' value='".$cod."'><br>

                        <b>Descrição:</b></td></tr>".$erro."<br>
                        <input type='text' name='desc' value='".$desc."'><br>

                        <b>Nota de transferência:</b>".$erro."<br>
                        <input type='text' name='notaf' value='".$nota."'><br>

                        <b>Data da nota:</b>".$erro."<br>
                        <input type='text' name='datanota' value='".$data."'><br>

                        <b>Loja origem:</b>".$erro."<br>
                        <input type='text' name='loja' value='".$loja."'><br>

                        <b>Avaliação / Loja</b><br>
                        <input type='text' name='valorloja' value='".$vloja."'><br>

                        <b>Avaliação / AT</b><br>
                        <input type='text' name='valorat' value='".$vat."'><br>

                        <b>Capacidade da bateria (%)</b><br>
                        <input type='text' name='capacidadebateria' value='".$bat."'><br>
                        ...
                        <textarea name='obs'></textarea>
                          ...
                          <checkbox>
                              <b>Estado Físico do Produto</b>".$erro."<br><br>
                              <input type='checkbox' id='estado1' name='estado1' value='APARELHO EM BOAS CONDICOES, '>
                              <label for='estado1'>APARELHO EM BOAS CONDICOES</label><br>
                      ...
                      <send><button type='submit' name='register'>SALVAR</button></send>
                  </content>
               </form>
            ";
